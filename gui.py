...
class Gui1:
    ...
    def __init__(self, master, web_page):
        ...
        # self.my_button = Button(master, text='send', command=self.clicker)
        # self.my_button.pack()
    ...
    def binding(self):
        ...
        self.doc_entry.bind("<KeyRelease>", lambda event: self.clicker())
        self.doc_list.bind('<<ListboxSelect>>', lambda event: self.fill_entry())

    def fill_entry(self):
        self.doc_entry.delete(0, END)
        self.doc_entry.insert(0, self.doc_list.get(ACTIVE))


def input_data(string, web_page):
    if __name__ == '__main__':
        str_list = ['שובל', 'שיר', 'שובר']
        return str_list
